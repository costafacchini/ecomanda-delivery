...
Delete `src/app/helpers/FractionalProducts.js` and its test file `src/app/helpers/FractionalProducts.spec.js`. This helper was only used by the PDV cart layer, which has already been deleted.

## Context

`FractionalProducts.js` calculates whether a product quantity should be treated as fractional (decimal) or integer, a concept that only applied to PDV cart items. With the cart and order models removed, this helper is dead code.
...
| File | Action | Notes |
|------|--------|-------|
| `src/app/helpers/FractionalProducts.js` | delete | Dead code — only used by PDV cart layer |
| `src/app/helpers/FractionalProducts.spec.js` | delete | Test file for deleted helper |
...
Remove both `FractionalProducts.js` and `FractionalProducts.spec.js`.
...
- [ ] `FractionalProducts.js` deleted
- [ ] `FractionalProducts.spec.js` deleted
