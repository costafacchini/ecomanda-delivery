...
Remove the `parseCart` function from `src/app/helpers/ParseTriggerText.ts`. Update `ParseTriggerText.spec.ts` to remove tests for `parseCart`. Verify no remaining callers exist.

## Context

`ParseTriggerText.ts` is a shared helper that parses trigger text tokens. The `parseCart` function within it was PDV-specific, converting trigger tokens into cart item structures. The rest of the helper (non-cart parsing) is still in use.
...
- [ ] Read `ParseTriggerText.ts` in full before editing
...
| File | Action | Notes |
|------|--------|-------|
| `src/app/helpers/ParseTriggerText.ts` | modify | Remove `parseCart` function only |
| `src/app/helpers/ParseTriggerText.spec.ts` | modify | Remove test cases for `parseCart` |

## Implementation Steps

### Step 1: Read ParseTriggerText.ts
...
### Step 3: Remove parseCart from ParseTriggerText.ts
...
### Step 4: Update ParseTriggerText.spec.ts
...
- [ ] `npx jest src/app/helpers/ParseTriggerText.spec.ts` passes
- [ ] `npx jest` full suite passes
- [ ] `pre-commit-check` passes

## Completion Criteria

- [ ] `parseCart` function removed from `ParseTriggerText.ts`
