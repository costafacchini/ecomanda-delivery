      ...
      it('returns status 200 and message if licensee exists', async () => {
        await request(expressServer)
          ...
          .then((response) => {
            ...
            expect(response.body[4].name).toEqual('Alcateia Ltds')
            expect(response.body[4].email).toEqual('alcateia@alcateia.com')
            expect(response.body[4].phone).toEqual('11098538273')
            expect(response.body[4].active).toEqual(true)
            expect(response.body[4].licenseKind).toEqual('demo')
            expect(response.body[4].useChatbot).toEqual(true)
            expect(response.body[4].chatbotDefault).toEqual('landbot')
            expect(response.body[4].whatsappDefault).toEqual('winzap')
            expect(response.body[4].chatDefault).toEqual('rocketchat')
            expect(response.body[4].chatbotUrl).toEqual('https://chatbot.url')
            expect(response.body[4].chatbotAuthorizationToken).toEqual('chat-bot-token')
            expect(response.body[4].whatsappToken).toEqual('whatsapp-token')
            expect(response.body[4].whatsappUrl).toEqual('https://api.winzap.com.br/send/')
            expect(response.body[4].chatUrl).toEqual('https://chat.url')
            expect(response.body[4].awsId).toEqual('aws-id')
            expect(response.body[4].awsSecret).toEqual('aws-secret')
            expect(response.body[4].bucketName).toEqual('bocket-name')
            expect(response.body[4]._id).toBeDefined()
            expect(response.body[4]._id).not.toBe('')
            expect(response.body[4]._id).not.toBe(null)
          })
      })
