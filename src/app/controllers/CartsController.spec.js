    ...
    describe('response', () => {
      it('returns status 200 and the cart data if the update is successful', async () => {
        ...
        await request(expressServer)
          ...
          .expect(200)
          .then((response) => {
            expect(response.body.total).toEqual(7.1)
            expect(response.body.concluded).toEqual(true)
            expect(response.body.contact).toEqual(contact._id.toString())
            expect(response.body.licensee).toEqual(licensee._id.toString())

            expect(response.body.products.length).toEqual(1)
            expect(response.body.products[0].product_retailer_id).toEqual('0124')
            expect(response.body.products[0].name).toEqual('Product 2')
            expect(response.body.products[0].quantity).toEqual(1)
            expect(response.body.products[0].unit_price).toEqual(1.0)

            expect(response.body.products[0].additionals.length).toEqual(1)
            expect(response.body.products[0].additionals[0].name).toEqual('Additional 2')
            expect(response.body.products[0].additionals[0].quantity).toEqual(2)
            expect(response.body.products[0].additionals[0].unit_price).toEqual(0.7)

            expect(response.body.products[0].additionals[0].details.length).toEqual(1)
            expect(response.body.products[0].additionals[0].details[0].name).toEqual('Detail 2')
            expect(response.body.products[0].additionals[0].details[0].quantity).toEqual(2)
            expect(response.body.products[0].additionals[0].details[0].unit_price).toEqual(0.8)
          })
      })
