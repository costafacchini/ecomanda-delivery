...
function makeSession(overrides: Record<string, any> = {}) {
  return { _id: 'session-id-1', licensee: 'licensee-id-1', inbox: null, creds: {}, keys: {}, ...overrides }
}
...
describe('BaileysSocketManager', () => {
  ...
  describe('isConnectedForLicensee()', () => {
    ...
    it('returns true for the main session (inbox=null) after connection opens', async () => {
      await manager.start(session, licensee, {})
      fireEvent('connection.update', { connection: 'open' })
    ...
    it('returns true for an inbox session after connection opens', async () => {
      const inboxId = 'inbox-id-abc'
      const inboxSession = makeSession({ _id: 'session-id-2', inbox: inboxId })
      await manager.start(inboxSession, licensee, {})
      fireEvent('connection.update', { connection: 'open' })

      expect(manager.isConnectedForLicensee('licensee-id-1', inboxId)).toBe(true)
    })

    it('does not confuse main session with inbox session', async () => {
      const inboxId = 'inbox-id-abc'
      ...
      expect(manager.isConnectedForLicensee('licensee-id-1', null)).toBe(true)
      expect(manager.isConnectedForLicensee('licensee-id-1', inboxId)).toBe(false)
    })
    ...
    it('stores two sessions for the same licensee under different keys', async () => {
      ...
      expect(manager.isConnected(inboxSession._id)).toBe(true)
    })
  })
})
