...
const getTemplates = async (url, token) => {
  ...
  try {
    ...
    logger.error({ err: error }, 'YCloud - erro: Erro ao buscar templates:')
    return { templates: [] }
  }
}
...
class YCloud extends MessengersBase {
  ...
  parseContactData(responseBody) {
    ...
    logger.info({ chatId }, 'YCloud - contato recebido')
  ...
  async sendMessage(messageId, url, token) {
    ...
    try {
      ...
      await messageToSend.save()
      logger.error({ err: error }, `YCloud - erro: Erro ao enviar mensagem ${messageId} para YCloud:`)
    }
  }
  ...
  async setWebhook(url, token) {
    ...
    try {
      ...
      logger.error({ err: error }, 'YCloud - erro: Erro ao configurar webhook:')
      return false
    }
  }
