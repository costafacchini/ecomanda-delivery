...
describe('IngestMessengerMessage', () => {
  ...
  it('saves the body with department null when departmentId is not provided', async () => {
    const messengerRepository = new BodyRepositoryMemory()
    const jobQueue = {
      addJob: jest.fn().mockResolvedValue(undefined),
    }
  ...
  it('saves the body with department populated when departmentId is provided', async () => {
    const messengerRepository = new BodyRepositoryMemory()
    const jobQueue = {
      addJob: jest.fn().mockResolvedValue(undefined),
    }
